var ChatApp = function(user,socket,server){ // tap trung xu ly, dong bo du lieu,
    ...
    var URL_UPLOAD = "http://localhost:3000/api/upload",
    ...
    var _uploadFileAttach = function(fileList,callback){
        var fd = new FormData();
        fd.append("recfile", fileList);
        var xhr = new XMLHttpRequest();
        xhr.responseType = 'json';
    ...
    var _showContentMessage = function(content){
        for(var i = 0;i<EMOTICON_BASIC_SHORTCUT.length;i++){
            content = _.replace(content,EMOTICON_BASIC_SHORTCUT[i],_imgEmoticon(EMOTICON_BASIC_IMAGE[i]));
        }
        return content;
    }
    var _loadNewMessage = function(user,state,callback){
        var _this = this;
    ...
    this._contacts = _get_localStorage(STORAGE_CONTACT); // contactId:{id, nickName, last, avatar, last, state}
            ...
            if(_this.box.type === "group"){
                    ...
                    groupName.dblclick(function(){
                        ...
                        $(this).keydown(function (e) { 
                            if (e.which == 13) {
                                e.preventDefault();
                                if ($(this).html().trim() !== '' ) {
                                    _updateGroup({id:_this.box.id,title:$(this).html().trim()},UPDATE_GROUP,function(group){
                                        _callback(UPDATE_GROUP_LOCAL,group);
                                    });
                ...
                search_btnUpdate.click(function(){
                    ...
                    _updateGroup({id:_this.box.id,member:memberUpdate},UPDATE_GROUP,function(g){
                        search_content.removeClass("view-adduser");
                        _this.box_content_top.find(".ga").fadeOut(1000); 
                        _callback(UPDATE_GROUP_LOCAL,g);
                    });
                });
    ...
    var ChatContact = function(state, title){
        ...
        this.contact_tab_recently = $("<div id=\"tab-recently\" class=\"tab-pane fade\"></div>");
        ...
        // this.tab = [this.contact_tab_online,this.contact_tab_recently,this.contact_tab_phancap,this.contact_tab_friend,this.contact_tab_nhom];
        ...
        // *** contructor element
        this.createContactor = function(id){
            var _this = this;
            var contact = _contacts[id]
            if(!contact || id == _user.id){
                return undefined;
            }
            ...
            contactor.click(function(){
                ...
                console.log("app",_app._contacts,_contacts);
            });
        ...
        this.contactor = function(id,tab){ // create or update contactor.
            ...
            return _this.createContactor(id);
            
        };
        this.nhomContactor = function(id,idmember){
            var _this = this;
        ...
        this.groupDom = function(group){ // {id,member,title}
            var _this = this;
            if(!_this.nhomDom[group.id])
                _this.nhomDom[group.id] = {};
            for(var key in _this.nhomDom[group.id])
                _this.domStore.append(_this.nhomDom[group.id][key]);
            var groupDom = _this.panelCollapor(group.name,group.id);
            group.member.forEach(function(m){
                groupDom.panelContent.append(_this.nhomContactor(group.id,m));
            });
        ...
        this.loadRecently = function(recently){ // [{id,type:"group|user"}]
            var _this = this;
            var recentlyDom = $("<div class=\"live-chat list contact-chat\">");
            if(recently){
            }
            _this.contact_tab_recently.find(".live-chat").remove();
            _this.contact_tab_recently.append(recentlyDom);
        };
        ...
        this.loadOnline = function(onl){ //add contactor online.
            ...
            if(_this.contact_tab_online.find(".live-chat").length == 0)
                _this.contact_tab_online.append("<div class=\"live-chat list contact-chat\">");
        ...
        this.loadNhom = function(nhoms){//[{id,name,members}]
            ...
            for(var key in _this.nhomDom){
                _this.domStore.append(_this.nhomDom[key]);
            }
        ...
        this.init = function(){
            ...
            btn_addgroup.click(function(){
                _callback(CHATCONTACT_CREATEGROUP);
            });
            // load tab content;
    ...
    var _callback = function(type,data){
        ...
        if(type === CHATCONTACT_CREATEGROUP){
            _updateGroup({creater: _user.id},CREATE_GROUP,function(g){
                ...
                _listBox[g.id] = new ChatBox(bg,{show:true, full:true});
                _dom.append(_listBox[g.id].dom);
                _showBox(g.id,true);
            })
        }
        if(type === UPDATE_GROUP_LOCAL){
