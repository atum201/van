...
var _env = require('./backend/business/env');
...
var _express = require('./backend/business/express');
...
var _variable = require('./backend/common/variable');

var _mongodb = require('./backend/mongodb');

var _constant = require('./backend/common/constant');

var _util = require('./backend/common/util');
...
var randomVersion = "";
_mongodb.PhanCap.findOne({}).sort({ time: -1 }).execAsync().then(function (phancap) {
...
io.on('connection', function (socket) {
  socket.on(_constant.SOCKET_SEND_CONNECT, function (data) {
    // user {userId}
    _mongodb.User.findOneAsync({ userId: data.userId }).then(function (user) {
      ...
      if (user.first) {
        ...
        _mongodb.User.updateAsync({ userId: data.userId }, { $set: { login: login, last: new Date().getTime() } });
      } else {
        // first login.
  ...
  socket.on(_constant.SOCKET_SEND_MESSAGE, function (data) {
    ...
    var message = new _mongodb.Message({
      from: data.from ? ObjectID(data.from) : undefined,
      toGroup: data.toGroup ? ObjectID(data.toGroup) : undefined,
      toUser: data.toUser ? ObjectID(data.toUser) : undefined,
    ...
    message.saveAsync().then(function (message) {
      ...
      if (message.toUser) {
        ...
        _mongodb.User.findOneAsync({ _id: ObjectID(message.toUser) }).then(function (user) {
      ...
      if (message.toGroup) {
        ...
        type = "group";
        _mongodb.Group.findOneAsync({ _id: ObjectID(message.toGroup) }).then(function (group) {
          _variable.socker.forEach(function (sk) {
          ...
          _mongodb.User.findAsync({ _id: { $in: idsIn } }).then(function (users) {
      ...
      _mongodb.User.findOneAsync({ _id: ObjectID(message.from) }).then(function (user) {
        if (!user.recent) {
          user.recent = [];
        }
  ...
  socket.on(_constant.SOCKET_SEND_UPDATE_GROUP, function (data) {
    // data: groupId
    _mongodb.Group.findOneAsync({ _id: ObjectID(data) }).then(function (group) {
      _variable.socker.forEach(function (sk) {
  ...
  socket.on(_constant.SOCKET_SEND_UPDATE_USER, function (data) {
    // data: userId.
    _mongodb.User.findOneAsync({ _id: ObjectID(data) }).then(function (user) {
      _variable.socker.forEach(function (sk) {
  ...
  socket.on(_constant.SOCKET_SEND_NOTIFY, function (receiver, data) {
    _mongodb.User.findAsync({ userId: { $in: receiver } }, { id: 1 }).then(function (ids) {
      _variable.socker.forEach(function (sk) {
  ...
  socket.on(_constant.SOCKET_SEND_READ_MESSAGE, function (data) {
    ...
    _mongodb.User.findOneAsync({ _id: ObjectID(data.id) }).then(function (user) {
  ...
  socket.on(_constant.SOCKET_SEND_LOGIN_STORE, function (data) {
    ...
    var year = data.year;
    var result = 0;
    _mongodb.User.findOneAsync({ userId: data.username }).then(function (user) {
      if (user) {
  ...
  socket.on(_constant.SOCKET_SEND_MANY_LOGIN_STORE, function (data) {
    ...
    var year = data.year;
    var result = [];
    _mongodb.User.findAsync({ userId: { $in: data.users } }).then(function (users) {
      if (users) {
        users.forEach(function (user) {
